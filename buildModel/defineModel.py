...
conv3a_weight = tf.get_variable("Conv_3A", shape=[4, 96, conv3a_features],
                                initializer=tf.contrib.layers.xavier_initializer())
conv3a_bias = tf.Variable(tf.zeros([conv3a_features], dtype=tf.float32))
conv3b_weight = tf.get_variable("Conv_3B", shape=[2, 96, conv3b_features],
                                initializer=tf.contrib.layers.xavier_initializer())
conv3b_bias = tf.Variable(tf.zeros([conv3b_features], dtype=tf.float32))
convMax3_weight = tf.get_variable("Conv_max_W3", shape=[1, 96, convMax3_features],
                                  initializer=tf.contrib.layers.xavier_initializer())
convMax3_bias = tf.Variable(tf.zeros([convMax3_features], dtype=tf.float32))
convAvg3_weight = tf.get_variable("Conv_avg_W3", shape=[1, 96, convAvg3_features],
                                  initializer=tf.contrib.layers.xavier_initializer())
convAvg3_bias = tf.Variable(tf.zeros([convAvg3_features], dtype=tf.float32))

conv4a_weight = tf.get_variable("Conv_4A", shape=[4, 256, conv4a_features],
                                initializer=tf.contrib.layers.xavier_initializer())
conv4a_bias = tf.Variable(tf.zeros([conv4a_features], dtype=tf.float32))
conv4b_weight = tf.get_variable("Conv_4B", shape=[2, 256, conv4b_features],
                                initializer=tf.contrib.layers.xavier_initializer())
conv4b_bias = tf.Variable(tf.zeros([conv4b_features], dtype=tf.float32))
convMax4_weight = tf.get_variable("Conv_max_W4", shape=[1, 256, convMax4_features],
                                  initializer=tf.contrib.layers.xavier_initializer())
convMax4_bias = tf.Variable(tf.zeros([convMax4_features], dtype=tf.float32))
convAvg4_weight = tf.get_variable("Conv_avg_W4", shape=[1, 256, convAvg4_features],
                                  initializer=tf.contrib.layers.xavier_initializer())
convAvg4_bias = tf.Variable(tf.zeros([convAvg4_features], dtype=tf.float32))

conv5a_weight = tf.get_variable("Conv_5A", shape=[2, 640, conv5a_features],
                                initializer=tf.contrib.layers.xavier_initializer())
conv5a_bias = tf.Variable(tf.zeros([conv5a_features], dtype=tf.float32))
conv5b_weight = tf.get_variable("Conv_5B", shape=[2, 640, conv5b_features],
                                initializer=tf.contrib.layers.xavier_initializer())
conv5b_bias = tf.Variable(tf.zeros([conv5b_features], dtype=tf.float32))
convMax5_weight = tf.get_variable("Conv_max_W5", shape=[1, 640, convMax5_features],
                                  initializer=tf.contrib.layers.xavier_initializer())
convMax5_bias = tf.Variable(tf.zeros([convMax5_features], dtype=tf.float32))
convAvg5_weight = tf.get_variable("Conv_avg_W5", shape=[1, 640, convAvg5_features],
                                  initializer=tf.contrib.layers.xavier_initializer())
